...
- 客户端本地多目标核心：shared-parameter `UPGrad-JD`
- 本地求解：每个 batch 精确重算 UPGrad
- 目标尺度：per-objective gradient normalization
- 全局校正：服务器根据 validation RI 广播 task progress weights `q`
- 通信：客户端仅上传 `Δθ`
- 服务器更新：sample-weighted FedAvg，不启用 `align_scores` 或 global momentum

`AdaptiveRescaling`、local/global momentum、`align_scores`、stochastic Gramian 和 weight reuse 暂时作为后续 ablation/效率模块，不混入当前主表默认结果。

### 4.1 对比方法

| 方法 | 说明 | 来源绑定 | local_epochs |
|------|------|---------|-------------|
| **NFJD** | global-progress-aware shared-UPGrad + Δθ 上传 + FedAvg 聚合 | 本仓库方法 | 3 |
...
7. 优先评估 global progress weights 是否改善 MultiMNIST Non-IID 的 task 0 退化；之后再考虑 AdaptiveRescaling、local/global momentum、alignment-aware weighting、stochastic Gramian 或 weight reuse。
...
| use_global_progress_weights | True | N/A |
| progress_beta | 2.0 | N/A |
| progress_weight_clip | [0.5, 2.0] | N/A |
...
**关键预期**：
- NFJD 主线验证 global-progress-aware shared-UPGrad 是否能同时保持 IID 优势并缓解 Non-IID task 0 退化
- Phase 5 重点验证“同级通信预算下的性能优势”，而不是依赖 Jacobian 上传差距
- 通过 `avg_task_weight_gap` 记录全局任务进度校正强度，避免事后无法追溯
...
**关键预期**：
- NFJD 与正式 baseline 具有相近上传量，但应先验证 global progress weights 是否改善测试指标和任务公平性
- stochastic / momentum / alignment 等增强项暂不作为主表默认配置
- River Flow 的任务量纲不一致更强，因此 objective normalization 与 progress weights 的共同效果是当前最关键观察点之一
