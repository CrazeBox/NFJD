...
4. **与文献基线对齐**：Phase 5 主表 baseline 将绑定到 MGDA-UB、PCGrad、CAGrad、线性标量化等有明确论文/官方代码来源的方法
...
| **FedAvg+MGDA-UB** | 本地 MGDA-UB min-norm 方向 | Sener & Koltun 2018 + 官方仓库 | 3 |
| **FedAvg+PCGrad** | 本地 gradient surgery | Yu et al. 2020 + 官方仓库 | 3 |
| **FedAvg+CAGrad** | 本地 conflict-averse 梯度组合 | Liu et al. 2021 + 官方仓库 | 3 |

说明：

1. Phase 5 的 baseline 不再使用 `fedjd / fmgda / weighted_sum / direction_avg / stl` 作为主表方法。
2. Phase 5 统一采用“**官方多任务优化算法 + 统一联邦 FedAvg wrapper**”的形式，以保证来源清晰、预算一致、结果表述诚实。
...
| 变量 | 取值 |
|------|------|
| 方法 | NFJD, FedAvg+LS, FedAvg+MGDA-UB, FedAvg+PCGrad, FedAvg+CAGrad |
| 数据划分 | IID |
| 客户端数 | 10 |
| 参与率 | 0.5 |
| 种子数 | 3 (7, 42, 123) |
...
| 变量 | 取值 |
|------|------|
| 方法 | NFJD, FedAvg+MGDA-UB, FedAvg+CAGrad |
| 数据划分 | IID |
| 任务数 | 2, 4, 6 |
| 种子数 | 3 |
...
| 参数 | NFJD | FedAvg+LS / MGDA / PCGrad / CAGrad |
|------|------|------------------------------------|
| num_rounds | 50 | 50 |
| local_epochs | 3 | 3 |
...
| num_clients | 10 | 10 |
| 联邦外壳 | NFJD Δθ aggregation | sample-weighted FedAvg on local deltas |
| 本地多目标规则 | NFJD | LS / MGDA-UB / PCGrad / CAGrad |
...
| 对比 | 指标 | 预期方向 | 验证方式 |
|------|------|----------|----------|
| NFJD vs FedAvg+MGDA-UB | Avg Accuracy | NFJD > baseline | Wilcoxon p < 0.05 |
...
| 对比 | 指标 | 预期方向 | 验证方式 |
|------|------|----------|----------|
| NFJD vs FedAvg+MGDA-UB | Avg MSE / Avg F1 | NFJD 更优 | Wilcoxon p < 0.05 |
