...
路径说明：默认数据目录按项目根目录解析。也可以设置 `FEDJD_DATA_ROOT=/path/to/data` 将所有默认数据目录移到服务器共享盘。
...
CelebA 建议手动准备，因为 torchvision 的 Google Drive 自动下载经常受配额或确认页影响。

去官网下载并找到以下三部分：

1. 打开 CelebA 官方页面：`https://mmlab.ie.cuhk.edu.hk/projects/CelebA.html`。
2. 在下载表的 `Img` 部分，下载 `Align&Cropped Images` 对应的 `img_align_celeba.zip`。
3. 在 `Anno` 部分，下载 `Attributes Annotations` 对应的 `list_attr_celeba.txt`。
4. 在 `Eval` 部分，下载 `Train/Val/Test Partitions` 对应的 `list_eval_partition.txt`。
5. 解压图片，并把图片目录和两个文本文件放到 `data/celeba/`，或放到自定义目录后设置 `CELEBA_ROOT=/path/to/celeba`。
...
如果解压后多了一层 `img_align_celeba/img_align_celeba/`，当前加载器也能识别。
