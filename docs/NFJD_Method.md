...
- global task correction: server-maintained task progress weights broadcast to clients
...
| Non-IID下本地视角偏置 | global task progress weights | §2.4 |
| 通信开销大 | 仅上传 Δθ | §2.2 |
...
NFJDServer:
  1. 客户端采样
  2. 根据全局验证任务进度计算 task weights q
  3. 模型 + q 分发 → 客户端本地训练
  4. 收集 Δθ
  5. 按样本数加权聚合 Δθ
  6. θ_global += aggregated_delta
...
NFJDClient:
  对每个本地 epoch:
    对每个 mini-batch:
      ...
      用全局 task weights q 加权各目标梯度
      用 UPGrad 求 shared 公共方向
      shared params: θ_s = θ_s - lr * direction
      head params: θ_h = θ_h - lr * grad(sum(q_i * loss_i))
  上传: Δθ
```
...
- task-specific heads 不参加共同方向求解，而是按全局任务权重加权后的总损失做普通梯度更新
- 最终仅上传 Δθ
...
### 2.4 Global Task Progress Weights

当前主线把联邦系统视为一个全局多目标问题：

```text
min_theta [F_1(theta), ..., F_m(theta)]
```

直接在服务器执行全局 UPGrad 需要上传 task-wise Jacobian，通信代价为 `m*d`。因此 NFJD 不在服务器求全局方向，而是让服务器维护一个轻量的任务进度权重向量：

```text
q = [q_1, ..., q_m]
```

服务器每轮根据验证集全局 objectives 计算相对改进：

```text
RI_i^t = (L_i^0 - L_i^t) / |L_i^0|
deficit_i^t = mean(RI^t) - RI_i^t
q_i^t = clip(exp(beta * deficit_i^t), q_min, q_max)
q^t = q^t / mean(q^t)
```

含义：

- 如果任务 `i` 的全局改进低于平均水平，`q_i > 1`
- 如果任务 `i` 的全局改进高于平均水平，`q_i < 1`
- `q` 被归一化到均值为 1，避免整体学习率漂移
...
g_i = g_i / EMA(||g_i||)
g_i = q_i * g_i
d_phi = UPGrad(g_1, ..., g_m)
```

任务头也使用同一权重：

```text
head_loss = sum_i q_i * L_i
```

这个机制不需要上传 Jacobian 或 `align_scores`，只增加服务器到客户端的 `m` 维广播，通信量可忽略。
...
### 2.6 Optional Extensions (Deferred)
...
| 方向尺度处理 | 无 | objective normalization + global progress weights |
| 动量 | 无 | 默认关闭 |
...
| Global progress weights | 服务器广播m维任务权重 | 校正Non-IID下本地任务视角偏置 |
...
| use_global_progress_weights | 是否使用全局任务进度权重 | True |
| progress_beta | 任务进度权重强度 | 2.0 |
| progress_min_weight | 任务权重下限 | 0.5 |
| progress_max_weight | 任务权重上限 | 2.0 |
...
| 任务权重差距 | Task Weight Gap | max(q)-min(q)，用于观察全局任务进度校正强度 |
...
|-----------|--------|-------------|
| 收敛速度慢于基线 | 高 | 当前先保留 exact 主干，后续再做 recompute/stochastic 效率版 |
| 通信开销随m线性增长 | 高 | Δθ上传，通信量与m解耦 |
| 计算复杂度高于基线 | 中 | 后续引入权重复用 / stochastic Gramian |
| m=10时性能严重退化 | 高 | StochasticGramianSolver（暂作为后续效率模块） |
| 高冲突/Non-IID场景任务偏置 | 高 | Global task progress weights |
...
1. Validate the global-progress-aware shared-UPGrad backbone on Phase 5, starting with MultiMNIST Non-IID
2. Sweep global progress weight parameters: `progress_beta`, `progress_min_weight`, `progress_max_weight`
3. Compare exact vs approximate local UPGrad only after the new backbone result is clear
4. Re-introduce optional modules one at a time:
   `adaptive rescaling`, `local momentum`, `global momentum`, `alignment-aware weighting`, `stochastic Gramian`
5. Add sensitivity analysis for objective normalization and optional server heuristics
6. Decide whether older benchmark scripts should remain active or be archived as legacy

The current practical recommendation is to keep server-side aggregation plain FedAvg and use only the lightweight global task-progress vector as the global correction signal.
